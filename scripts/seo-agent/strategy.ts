...
export async function generateRecommendations(
  ...
  const response = await anthropic.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `You are a technical SEO consultant analysing the current state of saunders-simmons.co.uk, a web design and digital marketing agency targeting UK businesses.
...
1. **New pages to create** — Are there keywords that need dedicated service pages, comparison pages, or landing pages rather than blog posts? (e.g. location-specific landing pages, dedicated service pages for web design, SEO, etc.)
2. **Technical SEO improvements** — Missing structured data, meta tag improvements, internal linking gaps, page speed concerns, new schema markup opportunities
3. **Content gaps** — Topics or angles not covered by existing blog posts that would build authority
...
5. **Competitive opportunities** — Based on the competitor landscape, what should we prioritise?

Important:
- Be specific and actionable — "Update the homepage H2" is better than "Improve on-page SEO"
